...
Notification::Notification(Container* parent)
  ...
  if(Config::getBool("USE_TEXTURES")) {
    ...
    m_closeIcon->setTextureRect(Config::getTextureManager()->getRect(
      Config::getString("NOTIFICATION_CLOSE_ICONTEX"),
      Config::getString("NOTIFICATION_CLOSE_ICONTEX_RECT"),
      0));
  } else {
    m_closeShape = new sf::RectangleShape();
    m_closeShape->setOutlineThickness(1);
    m_closeShape->setOutlineColor(
      Config::getColor("NOTIFICATION_CLOSE_OUTLINE"));
...
void
Notification::onUpdate(float frametime) {
  Clickable::onUpdate(frametime);
}
bool
Notification::onEvent(const sf::Event& e) {
  bool block = false;
  ...
  if(e.type == sf::Event::MouseMoved) {
    if(m_closeRect.contains(e.mouseMove.x, e.mouseMove.y)) {
      ...
      if(m_closeShape != 0) {
        m_closeShape->setFillColor(
          Config::getColor("NOTIFICATION_CLOSE_FILL_HOVER"));
        m_closeShape->setOutlineColor(
          Config::getColor("NOTIFICATION_CLOSE_OUTLINE_HOVER"));
      } else {
        m_closeIcon->setTextureRect(Config::getTextureManager()->getRect(
          Config::getString("NOTIFICATION_CLOSE_ICONTEX"),
          Config::getString("NOTIFICATION_CLOSE_ICONTEX_RECT_HOVER"),
          0));
      }
    } else {
      if(m_closeShape != 0) {
...
void
Notification::setBoundingBox(const sf::FloatRect& box) {
...
Notification::updateSize() {
  ...
  m_icon->setPosition(m_boundingBox.left + 5,
                      m_boundingBox.top + m_boundingBox.height / 2 -
                        m_icon->getGlobalBounds().height / 2);
  m_title->setBoundingBox(
    sf::FloatRect(m_boundingBox.left + 5 + m_icon->getGlobalBounds().width + 5,
                  m_boundingBox.top + 5,
  ...
  m_description->setBoundingBox(sf::FloatRect(
    m_boundingBox.left + 5 + m_icon->getGlobalBounds().width + 5,
    m_title->getBoundingBox().top + m_title->getBoundingBox().height + 15,
    m_boundingBox.width - m_icon->getGlobalBounds().width - 15,
    Config::getFloat("STANDARD_HEIGHT")));
  m_description->setWrap(m_boundingBox.width - m_icon->getGlobalBounds().width -
                         40);
  m_boundingBox.height = m_title->getBoundingBox().height +
                         m_description->getBoundingBox().height + 10;
...
void
Notification::fromNotificationData(const NotificationData& data) {
...
void
Notification::D_onClicked(const sf::Vector2f& relPos) {
  if(m_closeRect.contains(relPos.x + m_boundingBox.left,
                          relPos.y + m_boundingBox.top)) {
...
void
Notification::setTitle(const std::string& title) {
  m_title->setString(title);
  m_titleString = title;
}
void
Notification::setDescription(const std::string& description) {
  m_description->setString(description);
  m_descriptionString = description;
}
void
Notification::setIconTexture(const std::string& texture) {
  if(Config::getBool("USE_TEXTURES")) {
...
void
Notification::setIconTextureRect(const std::string& rect) {
  if(Config::getBool("USE_TEXTURES")) {
    m_textureRect = rect;
    m_icon->setTextureRect(Config::getTextureManager()->getRect(
      m_texture, m_textureRect, m_textureNumber));
  }
}
void
Notification::setIconTextureNumber(unsigned int number) {
  if(Config::getBool("USE_TEXTURES")) {
...
const std::string&
Notification::getIconTexture() {
  return m_texture;
}
const std::string&
Notification::getIconTextureRect() {
  return m_textureRect;
}
unsigned int
Notification::getIconTextureNumber() {
