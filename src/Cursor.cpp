...
void
Cursor::onUpdate(float frametime) {
...
bool
Cursor::onEvent(const sf::Event& e) {
  Widget::onEvent(e);
  return false;
}
void
Cursor::setBoundingBox(const sf::FloatRect& box) {
  Widget::setBoundingBox(box);
  m_backgroundRect->setPosition(sf::Vector2f(box.left, box.top));
  m_backgroundRect->setSize(
    sf::Vector2f(Config::getFloat("CURSOR_WIDTH"), box.height));
}
void
