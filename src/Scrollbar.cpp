...
void
Scrollbar::onUpdate(float frametime) {
  Clickable::onUpdate(frametime);
  m_scrolled = 0;
}
bool
Scrollbar::onEvent(const sf::Event& e) {
  return Clickable::onEvent(e);
}
void
Scrollbar::setBoundingBox(const sf::FloatRect& box) {
  Clickable::setBoundingBox(box);
}
void
Scrollbar::setScrollSpace(float space) {
  m_scrollSpace = space;
}
void
Scrollbar::setAlign(Scrollbar::Align align) {
  m_align = align;
  this->updateSize();
}
void
