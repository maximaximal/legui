...
void
PushButton::setBoundingBox(const sf::FloatRect& box) {
  Clickable::setBoundingBox(box);
  m_frame->setBoundingBox(box);
  m_label->setCharacterSize((unsigned int)box.height - 10);
  m_label->setBoundingBox(
    sf::FloatRect(m_boundingBox.left + m_boundingBox.width / 2 -
                    m_label->getBoundingBox().width / 2,
                  m_boundingBox.top + m_boundingBox.height / 2 -
                    m_label->getBoundingBox().height / 2,
                  m_label->getBoundingBox().width,
                  m_label->getBoundingBox().height));
}
void
PushButton::updateSize() {
  Clickable::updateSize();
  m_label->updateSize();
  m_frame->updateSize();
}
void
PushButton::draw(sf::RenderTarget& target, sf::RenderStates states) const {
