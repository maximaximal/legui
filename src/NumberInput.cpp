...
void
NumberInput::onUpdate(float frametime) {
...
bool
NumberInput::onEvent(const sf::Event& e) {
  bool block = false;
...
void
NumberInput::setBoundingBox(const sf::FloatRect& box) {
...
void
NumberInput::updateSize() {
  Clickable::updateSize();
  m_numberInput->updateSize();
}
void
NumberInput::D_onChanged(int newValue) {
  // To be derived.
}
void
NumberInput::draw(sf::RenderTarget& target, sf::RenderStates states) const {
