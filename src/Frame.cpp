...
void
Frame::setBoundingBox(const sf::FloatRect& box) {
...
void
Frame::draw(sf::RenderTarget& target, sf::RenderStates states) const {
  target.draw(*m_rectangleShape, states);
}
void
Frame::setOutlineColor(const sf::Color& color) {
  m_rectangleShape->setOutlineColor(color);
}
void
Frame::setFillColor(const sf::Color& color) {
  m_rectangleShape->setFillColor(color);
}
void
Frame::setOutlineThickness(float thickness) {
  m_rectangleShape->setOutlineThickness(thickness);
}
const sf::Color&
Frame::getOutlineColor() {
