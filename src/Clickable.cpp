...
void
Clickable::onUpdate(float frametime) {
...
bool
Clickable::onEvent(const sf::Event& e) {
  // Update the focusable widget.
    ...
    if(e.type == sf::Event::MouseButtonPressed) {
      if(e.mouseButton.button == sf::Mouse::Left) {
        if(m_boundingBox.contains(e.mouseButton.x, e.mouseButton.y)) {
          block = true;
          m_relPos = sf::Vector2f(e.mouseButton.x, e.mouseButton.y) -
                     sf::Vector2f(m_boundingBox.left, m_boundingBox.top);
          setPressed(true);
        }
      }
    }
...
void
Clickable::setPressed(bool state) {
  if(m_pressed) {
    if(!state) {
...
Nano::signal<void(const sf::Vector2f&)>&
Clickable::onPressed() {
  return m_onPressed;
}
Nano::signal<void(const sf::Vector2f&)>&
Clickable::onUnPressed() {
  return m_onUnPressed;
}
void
Clickable::D_onClicked(const sf::Vector2f& relPos) {}
void
Clickable::D_onReleased(const sf::Vector2f& relPos) {}
}
